...
- `src/services/HostPolicy.ts` — SSRF guard. `publicHostsOnly` resolves the hostname and rejects it if any address is loopback, private, link-local (cloud metadata), multicast or unspecified; `RequestFilter` applies the same policy to every request the page makes through Puppeteer request interception, so redirects and sub-resources are covered too. The policy is injected: `createAppFromConfig` defaults to `publicHostsOnly()`, and the tests pass `allowAllHosts` to render fixtures on `127.0.0.1`. Pass a `HostPolicy` explicitly wherever you construct `PuppeteerRenderingService`. Known gap: Puppeteer's interception rides on CDP's `Fetch` domain, which does not see WebSocket handshakes, so a page can still open a `ws://` connection to a private host.
